/**
 * events-live.js — Netlify Function
 *
 * GET  /.netlify/functions/events-live  → public read of live events from Netlify Blobs
 * POST /.netlify/functions/events-live  → admin write to Netlify Blobs (requires EVENTS_ADMIN_TOKEN)
 *
...
exports.handler = async (event) => {
    ...
    if (event.httpMethod === 'GET') {
        ...
        try {
            ...
            if (res.status === 404) {
                ...
                return { statusCode: 404, headers: corsHeaders, body: JSON.stringify({ error: 'No live events data yet.' }) };
            }
    ...
    if (event.httpMethod === 'POST') {
        ...
        if (!body.events || !Array.isArray(body.events)) {
            return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ error: 'Payload must contain an events array.' }) };
        }
        ...
        try {
            const res = await fetch(blobUrl(), {
                ...
                body: JSON.stringify({ events: body.events }, null, 2)
            });
